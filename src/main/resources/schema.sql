...
(
    id        integer default nextval('user_id_seq') not null constraint user_pk primary key,
...
(
    id  integer default nextval('"item_id_seq"') not null constraint item_pk primary key,
...
(
    id serial constraint bookings_pk primary key,
    booker_id  integer not null constraint bookings_users_id_fk references users
            on update cascade on delete cascade,
